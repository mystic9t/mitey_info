...
export default function Home() {
  ...
  return (
    <div className="min-h-[calc(100vh-140px)] relative bg-white">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-hidden">
        <Image
          ...
          width={1200}
          height={1200}
          className="w-full max-w-4xl h-auto grayscale opacity-[0.25]"
          priority={false}
        />
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-[calc(100vh-140px)] text-center px-4 py-8">
        <div className="animate-fade-in max-w-4xl w-full">
          {/* Brand Logo */}
          <div className="flex flex-col items-center mb-4">
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-saffron to-saffron/80 flex items-center justify-center shadow-xl mb-3">
              <svg className="w-12 h-12 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
              </svg>
            </div>
            <h2 className="text-2xl font-bold text-navy">{t.brand}</h2>
            <p className="text-saffron font-medium text-sm">{t.brandHi}</p>
          </div>

          {/* Scheme count badge */}
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-saffron/10 text-saffron rounded-full text-base font-medium mb-4">
          ...
          {/* Hero Title */}
          <h1 className="text-5xl font-bold text-navy mb-3 text-balance">
            {t.heroTitle}
            <span className="block text-saffron mt-1">{t.heroTitleHighlight}</span>
          </h1>
          
          {/* Subtitle */}
          <p className="text-xl text-muted-foreground max-w-lg mx-auto mb-6 text-balance">
            {t.heroSubtitle}
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-6">
            ...
            <Link href="/schemes" className="btn-outline text-lg px-8 py-4">
              <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              {categories.map((category) => {
                ...
                return (
                  <Link
                    ...
                    className={`category-pill border text-xs sm:text-base px-2 sm:px-4 py-2 ${info.color} w-full sm:w-auto whitespace-nowrap overflow-hidden`}
                  >
